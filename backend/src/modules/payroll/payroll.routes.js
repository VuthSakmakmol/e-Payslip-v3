...
import { readEmployeePdfPassword } from "../auth/pdfCredential.service.js";
...
import {
  importLocalPayroll,
  importForeignerPayroll,
} from "./services/payrollImport.service.js";
import {
  reconcilePayrollRows,
  publicReconciliation,
} from "./services/payrollReconciliation.service.js";
import {
  getTransientPayroll,
...
async function activeDesign() {
  const design = await PayslipDesign.findOne({ active: true }).sort({
    updatedAt: -1,
  });
  if (!design)
    throw new AppError(
      "No active payslip design. Create and activate a design first.",
      400,
    );
  return design;
}
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    if (!req.file?.buffer)
      throw new AppError("Payroll XLSX file is required", 400);
    const staffCategory = String(req.body.staffCategory || "").toUpperCase();
    const year = Number(req.body.year);
    const month = Number(req.body.month);

    if (!["LOCAL", "FOREIGNER"].includes(staffCategory))
      throw new AppError("staffCategory must be LOCAL or FOREIGNER", 400);
    if (!Number.isInteger(year) || year < 2000 || year > 2100)
      throw new AppError("Invalid payroll year", 400);
    if (!Number.isInteger(month) || month < 1 || month > 12)
      throw new AppError("Invalid payroll month", 400);
...
router.get(
  "/batches/:id",
  asyncHandler(async (req, res) => {
    const item = await PayrollBatch.findById(req.params.id).populate(
      "payPeriodId",
    );
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const records = await PayrollRecord.find({ batchId: batch._id }).sort({
      employeeCode: 1,
    });
...
router.get(
  "/batches/:id/preview/:employeeCode",
  asyncHandler(async (req, res) => {
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename="Payslip-${record.employeeCode}.pdf"`,
    );
    res.send(pdfBuffer);
  }),
);
...
router.post(
  "/batches/:id/release",
  asyncHandler(async (req, res) => {
    const batch = await PayrollBatch.findById(req.params.id).populate(
      "payPeriodId",
    );
    if (!batch) throw new AppError("Payroll batch not found", 404);
    if (batch.status === "RELEASED")
      throw new AppError("Payroll batch is already released", 409);
    if (batch.status !== "READY")
      throw new AppError("Payroll batch is not ready for release", 409);
    ...
    if (!reconciliation.clean) {
      throw new AppError(
        "Payroll reconciliation is not clean. Release is blocked.",
        409,
        {
...
router.get(
  "/transient/:id/preview/:employeeCode",
  asyncHandler(async (req, res) => {
    const session = getTransientPayroll(req.params.id);
    const row = session.rows.find(
      (item) => item.employeeCode === req.params.employeeCode,
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `inline; filename="Payslip-${row.employeeCode}.pdf"`,
    );
    res.send(pdfBuffer);
  }),
);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!reconciliation.clean) {
      throw new AppError(
        "Payroll reconciliation is not clean. Approval is blocked.",
        409,
        {
          reconciliation: publicReconciliation(reconciliation),
        },
    ...
    const session = approveTransientPayroll(
      req.params.id,
...
router.post(
  "/transient/:id/release",
  asyncHandler(async (req, res) => {
    const session = getTransientPayroll(req.params.id);
    if (!session.approved)
      throw new AppError(
        "Foreigner payroll must be previewed and approved before release",
        409,
      );
    const reconciliation = await reconcilePayrollRows({
      rows: session.rows,
    ...
    const [currentEmployees, pdfCredentials] = await Promise.all([
      Employee.find({ _id: { $in: employeeIds }, active: true }),
    ...
    const currentEmployeeMap = new Map(
      currentEmployees.map((employee) => [employee._id.toString(), employee]),
    );
    const pdfCredentialMap = new Map(
      pdfCredentials.map((credential) => [
    ...
    try {
      for (const row of session.rows) {
        ...
        if (!employee) {
          throw new AppError(
            `Employee ${row.employeeCode} is missing or inactive at release time`,
            409,
          );
        }
        let pdfPassword = "";
        if (
          employee.staffCategory === "FOREIGNER" &&
          employee.preferredDelivery === "EMAIL"
        ) {
          const credential = pdfCredentialMap.get(employee._id.toString());
          pdfPassword = readEmployeePdfPassword(credential) || "";
          if (!pdfPassword) {
            throw new AppError(
              `PDF password is missing for foreign email employee ${row.employeeCode}`,
              409,
            );
          }
        }
