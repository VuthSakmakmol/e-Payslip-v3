...
export async function reconcilePayrollRows({ rows, expectedCategory }) {
  ...
  const payrollCodes = new Set(
    rows.map((row) => normalizeCode(row.employeeCode)).filter(Boolean),
  );
  ...
  const [expectedEmployees, payrollCandidates] = await Promise.all([
    Employee.find({ staffCategory: category, active: true })
      .select(
        "_id employeeCode fullName dateJoin department position staffCategory active preferredDelivery companyEmail telegramChatId telegramProfile",
      )
      .lean(),
    codes.length
  ...
  const candidateMap = new Map(
    payrollCandidates.map((employee) => [
      normalizeCode(employee.employeeCode),
      employee,
    ]),
  );
  ...
  for (const row of rows) {
    ...
    if (candidate.active === false || candidate.staffCategory !== category) {
      ...
      if (candidate.active === false) {
        fields.push(
          fieldComparison("active", "Status", "Payroll Row", "Inactive", false),
        );
      }
    ...
    const comparisons = [
      fieldComparison(
        ...
        "Employee ID",
        code,
        normalizeCode(candidate.employeeCode),
        code === normalizeCode(candidate.employeeCode),
      ),
      fieldComparison(
        "dateJoin",
        "Date Join",
        payrollJoin,
        masterJoin,
        Boolean(payrollJoin && masterJoin && payrollJoin === masterJoin),
      ),
      fieldComparison(
        "department",
        "Department",
        ...
        Boolean(
          payrollDepartment &&
          masterDepartment &&
          normalizeText(payrollDepartment) === normalizeText(masterDepartment),
        ),
      ),
      fieldComparison(
        "position",
        ...
        Boolean(
          payrollPosition &&
          masterPosition &&
          normalizeText(payrollPosition) === normalizeText(masterPosition),
        ),
      ),
    ];
  ...
  const masterOnly = expectedEmployees.filter(
    (employee) => !payrollCodes.has(normalizeCode(employee.employeeCode)),
  );
  ...
  return {
    clean:
      summary.payrollOnly === 0 &&
      summary.masterOnly === 0 &&
      summary.mismatch === 0 &&
      summary.verified === rows.length,
    summary,
    issues,
    rows: rowStatuses,
    employeeByCode,
