<template>
  <div class="page-stack">
    <div class="metric-grid employee-metrics">
      ...
      <MetricTile
        ...
        :value="summary.telegramLinked"
        icon="pi pi-send"
        tone="blue"
      />
      <MetricTile
        label="First Login"
        :value="summary.firstLogin"
        icon="pi pi-key"
        tone="amber"
      />
    </div>
    ...
    <div class="surface-panel">
      <div class="app-toolbar">
        ...
        <div class="toolbar-right">
          <Button
            ...
            outlined
            :loading="exporting"
            v-tooltip.top="'Export credentials'"
      ...
      >
          ...
          <template #body="{ data }">
            <div class="identity-cell">
              ...
              <div class="identity-copy">
                ...
                <span
                  >{{ data.employeeCode
                  }}<template v-if="data.dateJoin">
                    · {{ formatDate(data.dateJoin) }}</template
                  ></span
                >
              </div>
            </div>
        ...
        <Column header="Category" style="min-width: 130px">
          <template #body="{ data }">
            <div class="tag-stack">
              <Tag
                :value="
                  data.staffCategory === 'FOREIGNER' ? 'Foreigner' : 'Local'
                "
                :severity="
                  data.staffCategory === 'FOREIGNER' ? 'info' : 'success'
                "
              />
            </div>
          </template>
        ...
        <Column header="Delivery" style="min-width: 225px">
          <template #body="{ data }">
            ...
            <button
              ...
              type="button"
              class="telegram-link"
              @click="openTelegramProfile(data)"
            >
              <span class="channel-icon telegram"
                ><i class="pi pi-send"
              /></span>
              <div class="stack-cell">
                <strong>{{
                  data.telegramSummary?.username
                    ? `@${data.telegramSummary.username}`
                    : "Telegram"
                }}</strong>
                <span>{{
                  data.telegramVerified ? "Verified" : "Not linked"
                }}</span>
              </div>
        ...
        <Column header="Credential" style="min-width: 145px">
          <template #body="{ data }">
            <code
              v-if="data.accessMode === 'PDF_ONLY' && data.pdfPassword"
              class="inline-code"
              >{{ data.pdfPassword }}</code
            >
            <code v-else-if="data.temporaryPassword" class="inline-code">{{
              data.temporaryPassword
            }}</code>
            <Tag
              v-else-if="data.accountStatus === 'ACTIVE'"
              value="Changed"
              severity="success"
            />
            <span v-else>—</span>
        ...
        <Column header="Account" style="min-width: 125px">
          <template #body="{ data }">
            <Tag
              :value="accountLabel(data)"
  ...
  <Dialog
    ...
    modal
    :header="form._id ? 'Edit Employee' : 'Add Employee'"
    :style="{ width: '760px', maxWidth: 'calc(100vw - 28px)' }"
  ...
  >
    ...
    <div class="form-section">
      <div class="form-section-title">
        <i class="pi pi-building" /> Organization
      </div>
      <div class="form-grid">
        <div class="form-field">
    ...
    <div class="form-section">
      ...
      <div class="form-grid">
        <div class="form-field">
          ...
          <Select
            v-model="form.preferredDelivery"
            :options="deliveries"
            optionLabel="label"
            optionValue="value"
          />
        ...
        <div class="form-field credential-mode">
          ...
          <Tag
            ...
            severity="info"
          />
          <Tag v-else value="Portal" icon="pi pi-user" severity="secondary" />
        </div>
      </div>
    </div>
  ...
  <Dialog
    v-model:visible="credentialDialog"
    modal
  ...
  <Drawer
    v-model:visible="telegramDialog"
    position="right"
    header="Telegram Profile"
    :style="{ width: '430px', maxWidth: '96vw' }"
  >
    <div v-if="telegramLoading" class="drawer-loading">
      <i class="pi pi-spin pi-spinner" />
    </div>
    <template v-else>
      <div class="profile-head">
        <Avatar
          :label="initials(telegramProfile.employee?.fullName)"
          shape="circle"
          size="large"
        />
        <div class="identity-copy">
          <strong>{{
            telegramProfile.employee?.fullName || "Employee"
          }}</strong>
          <span>{{ telegramProfile.employee?.employeeCode }}</span>
        </div>
        <Tag
          :value="telegramProfile.verified ? 'Verified' : 'Not Linked'"
      ...
      <div v-if="telegramProfile.profile" class="profile-list">
        <div>
          <span>Username</span
          ><strong>{{
            telegramValue(telegramProfile.profile.username, "@")
          }}</strong>
        </div>
        <div>
          <span>Name</span
          ><strong>{{ telegramFullName(telegramProfile.profile) }}</strong>
        </div>
        <div>
          <span>User ID</span
          ><strong>{{ telegramProfile.profile.userId || "—" }}</strong>
        </div>
        <div>
          <span>Chat ID</span
          ><strong>{{
            telegramProfile.activeChatId ||
            telegramProfile.profile.chatId ||
            "—"
          }}</strong>
        </div>
        <div>
          <span>Language</span
          ><strong>{{ telegramProfile.profile.languageCode || "—" }}</strong>
        </div>
        <div>
          <span>Chat Type</span
          ><strong>{{ telegramProfile.profile.chatType || "—" }}</strong>
        </div>
        <div>
          <span>Premium</span
          ><strong>{{
            telegramProfile.profile.isPremium ? "Yes" : "No"
          }}</strong>
        </div>
        <div>
          <span>Verified</span
          ><strong>{{
            formatDateTime(telegramProfile.profile.verifiedAt)
          }}</strong>
        </div>
        <div>
          <span>Last Seen</span
          ><strong>{{
            formatDateTime(telegramProfile.profile.lastSeenAt)
          }}</strong>
        </div>
      </div>
      <div v-else class="empty-state">
        <i class="pi pi-send" /><strong>Not linked</strong>
      </div>
    </template>
  </Drawer>
</template>
...
const telegramProfile = reactive({
  employee: null,
  verified: false,
  activeChatId: "",
  profile: null,
...
const isPdfOnlyForm = computed(
  () =>
    form.staffCategory === "FOREIGNER" && form.preferredDelivery === "EMAIL",
);
const credentialDialogTitle = computed(() =>
  credentials.type === "PDF_PASSWORD" ? "PDF Password" : "First Login",
);
...
function employeePayload() {
  return {
    ...
    companyEmail:
      form.preferredDelivery === "EMAIL"
        ? String(form.companyEmail || "").trim()
        : "",
...
function confirmResetPassword(employee) {
  const pdfOnly = employee.accessMode === "PDF_ONLY";
  confirm.require({
    header: pdfOnly ? "Reset PDF Password" : "Reset First Password",
    message: `Generate a new ${pdfOnly ? "PDF" : "first-login"} password for ${employee.fullName}?`,
    icon: "pi pi-key",
    rejectLabel: "Cancel",
    acceptLabel: "Reset",
...
async function resetPassword(employee) {
  try {
    const { data } = await api.post(
      `/employees/${employee._id}/reset-temporary-password`,
    );
    showCredentials({
...
async function openTelegramProfile(employee) {
  ...
  try {
    const { data } = await api.get(
      `/employees/${employee._id}/telegram-profile`,
    );
...
async function exportCredentials() {
  ...
  try {
    const response = await api.get("/employees/export-credentials", {
      responseType: "blob",
    });
...
async function copyCredentials() {
  const label =
    credentials.type === "PDF_PASSWORD" ? "PDF Password" : "First Password";
...
function accountLabel(employee) {
  if (employee.accountStatus === "NO_PORTAL") return "No Portal";
  if (employee.accountStatus === "FIRST_LOGIN") return "First Login";
  if (employee.accountStatus === "ACTIVE") return "Active";
  if (employee.accountStatus === "DISABLED") return "Disabled";
  if (employee.accountStatus === "LOCKED") return "Locked";
  return "Missing";
}
function accountSeverity(employee) {
  if (employee.accountStatus === "NO_PORTAL") return "info";
  if (employee.accountStatus === "ACTIVE") return "success";
  if (employee.accountStatus === "FIRST_LOGIN") return "warn";
  if (employee.accountStatus === "MISSING") return "danger";
  return "secondary";
}
function telegramValue(value, prefix = "") {
  return value ? `${prefix}${value}` : "—";
}
function telegramFullName(profile) {
  return (
